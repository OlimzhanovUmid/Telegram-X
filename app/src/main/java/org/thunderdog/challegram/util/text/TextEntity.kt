...
abstract class TextEntity(
  ...
  fun openParameters(view: View, text: Text?, part: TextPart, isFromLongPressMenu: Boolean): UrlOpenParameters {
    ...
    val b: TooltipBuilder? = if (isFromLongPressMenu) {
      UI.getContext(view.context).tooltipManager().builder(view)
    } else {
      part.newTooltipBuilder(view)
    }
  ...
  fun getTextPaint(textStyleProvider: TextStyleProvider, forceBold: Boolean): TextPaint {
    ...
    var storage = textStyleProvider.textPaintStorage
  ...
  storage = storage.getExtraBoldStorage();
    ...
    if (isUnderline) {
      storage = storage.underlineStorage
    }
    if (isStrikeThrough) {
      storage = storage.strikeThroughStorage
    }
    if (isSmall) {
  ...
  protected fun findRoot(view: View?): ViewController<*>? {
    ...
    if (c != null) return c
    return UI.getContext(view.context).navigation().currentStackItem
  }
  ...
  companion object {
    ...
    fun valueOf(context: ViewController<*>?, tdlib: Tdlib?, text: CharSequence?, openParameters: UrlOpenParameters?): Array<TextEntity?>? {
      ...
      val entities: MutableList<TextEntity?> = ArrayList()
    ...
    fun valueOf(tdlib: Tdlib?, `in`: String?, entities: Array<out TdApi.TextEntity?>?, openParameters: UrlOpenParameters?): Array<TextEntity?>? {
      ...
      val result: MutableList<TextEntity> = ArrayList()
      run {
        var i = 0
      ...
      for (i in 1..<result.size) {
        ...
        if (prev.end > entity.start) {
          Log.e("Error processing entities, textLength: %d, entities: %s", (`in`
            ?: "").length, entities)
    ...
    fun toEntities(text: CharSequence?): Array<TextEntity?>? {
      if (text is Spanned) {
        val entities = text.getSpans(0, text.length, TextEntity::class.java)
