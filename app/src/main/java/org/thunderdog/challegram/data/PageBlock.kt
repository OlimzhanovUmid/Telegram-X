...
abstract class PageBlock(@JvmField protected val context: ViewController<*>?, val originalBlock: TdApi.PageBlock?) {
  ...
  rectF.right = viewWidth - contentLeft;
      ...
      if (mergeBottom) {
        c.drawRect(rectF.left, rectF.bottom - lineWidth, rectF.right, view!!.measuredHeight.toFloat(), Paints.fillingPaint(lineColor))
      }
  ...
  class ParseContext(val url: String?, instantView: WebPageInstantView, val playListBuilder: PlayListBuilder?) {
    ...
    internal val viewCount: Int = instantView.viewCount
    ...
    internal fun processCaption(
      ...
      if (!caption.credit.isEmpty()) {
        ...
        if (captionBlock != null || needMerge) {
          credit.mergeWith((captionBlock ?: lastBlock)!!)
        }
        process(credit, out)
      }
    }
    ...
    private fun parse(
      ...
      when (block.getConstructor()) {
        PageBlockCover.CONSTRUCTOR -> {
          ...
          var i = index
          while (i < out.size) {
            if (out[i]!!.originalBlock === cover.cover) {
              context.coverBlock = out[i]
              break
            }
            i++
          }
        ...
        PageBlockList.CONSTRUCTOR -> {
          ...
          for ((itemIndex, item) in listRaw.items.withIndex()) {
            ...
            if (lastListItemInfo == null) {
              context.openedList = arrayOf(itemInfo)
            } else {
              context.openedList = arrayOfNulls(lastListItemInfo.size + 1)
              System.arraycopy(lastListItemInfo, 0, context.openedList.orEmpty(), 0, lastListItemInfo.size)
        ...
        PageBlockRelatedArticles.CONSTRUCTOR -> {
          ...
          for ((index, related) in relatedRaw.articles.withIndex()) {
            if (index > 0) {
