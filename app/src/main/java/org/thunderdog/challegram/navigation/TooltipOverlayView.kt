...
class TooltipOverlayView(context: Context?) : ViewGroup(context) {
    ...
    private class TooltipContentViewTextWrapper(parentView: TooltipOverlayView?, private val text: TextWrapper) : TooltipContentView(parentView) {
        ...
        override fun layout(info: TooltipInfo, parentWidth: Int, parentHeight: Int, maxWidth: Int): Boolean {
            text.setTextMediaListener(TextWrapper.TextMediaListener { wrapper: TextWrapper?, text: Text?, specificMedia: TextMedia? ->
        ...
        override fun layout(info: TooltipInfo, parentWidth: Int, parentHeight: Int, maxWidth: Int): Boolean {
            if (text == null || text!!.getMaxWidth() != maxWidth) {
                text = Text.Builder(
                    ...
                    info.colorProvider,
                    Text.TextMediaListener { text: Text?, specificMedia: TextMedia? ->
                        if (this.text === text) {
                            if (!text!!.invalidateMediaContent(info.iconReceiver, specificMedia)) {
                                text.requestMedia(info.iconReceiver)
                            }
    ...
    class TooltipLanguageSelectorView(parentView: TooltipOverlayView?, message: TGMessage, private val listener: OnClickListener) :
        ...
        init {
            ...
            translatedLanguage = Lang.getLanguageName(message.currentTranslatedLanguage, if (currentLang != null) currentLang else originalLanguage)
        ...
        override fun onTouchEvent(info: TooltipInfo, view: View?, e: MotionEvent?): Boolean {
            val isInside = info.isInside(e!!.getX(), e.getY())
            if (!isInside) {
                info.hide(true)
                return true
            }
    ...
    class TooltipInfo internal constructor(
        ...
        init {
            this.colorProvider = if (colorProvider != null) colorProvider else parentView.defaultProvider
            this.icon = Drawables.get(iconRes)
        ...
        fun layout(parentWidth: Int, parentHeight: Int): Boolean {
            ...
            if (view != null) {
                val scaleX = view.getScaleX()
                val scaleY = view.getScaleY()
                val pivotX = view.getPivotX()
                val pivotY = view.getPivotY()
        ...
        fun draw(c: Canvas) {
            ...
            if (Color.alpha(outlineColor) > 0) {
                ...
                parentView.paint.setStrokeWidth(Screen.dp(2f).toFloat())
        ...
        fun show(controller: ViewController<*>?, tdlib: Tdlib?, iconRes: Int, text: CharSequence): TooltipInfo? {
            if (originalView == null && viewProvider == null && locationProvider == null) {
                ...
                return icon(iconRes).needBlink(iconRes == R.drawable.baseline_info_24 || iconRes == R.drawable.baseline_error_24)
                    .controller(if (controller != null) controller.getParentOrSelf() else null).show(tdlib, text).hideDelayed(3500, TimeUnit.MILLISECONDS)
            }
        }
    ...
    fun reposition() {
        ...
        val viewHeight = getMeasuredHeight()
    ...
    private fun addHint(info: TooltipInfo) {
        for (i in activePopups.indices.reversed()) {
            val tooltipInfo = activePopups.get(i)
        ...
        when (e.getAction()) {
            MotionEvent.ACTION_DOWN -> {
                ...
                while (i >= 0) {
                    ...
                    } else if (hasFlag(info.flags, FLAG_INTERCEPT_TOUCH_EVENTS) && info.isInside(e.getX(), e.getY())) {
                        waitUp = true
    ...
    companion object {
        ...
        val FLAG_PREVENT_HIDE_ON_TOUCH: Int = 1 shl 1
        val FLAG_FILL_WIDTH: Int = 1 shl 2
        val FLAG_NO_PIVOT: Int = 1 shl 3
        val FLAG_NEED_BLINK: Int = 1 shl 4
        val FLAG_HANDLE_BACK_PRESS: Int = 1 shl 5
        val FLAG_IGNORE_VIEW_SCALE: Int = 1 shl 6
    }
}
