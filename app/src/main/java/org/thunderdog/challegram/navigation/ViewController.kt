...
import org.thunderdog.challegram.util.ColumnDataPicker.CommitListener
...
abstract class ViewController<T>(context: Context, @JvmField val tdlib: Tdlib?) : Future<View?>, ThemeChangeListener, Lang.Listener, ForceTouchView.StateListener,
    ...
    fun getParentOrSelf (): ViewController<*> {
        return (if (parentWrapper != null) parentWrapper else this)!!
    }
    ...
    fun bindThemeListeners(c: ViewController<*>?) {
        if (c != null) {
            ...
            if (this.themeListeners != null) c.themeListeners!!.getList().addAll(this.themeListeners!!.getList())
    ...
    protected open fun handleLanguageDirectionChange() {
        if (searchHeaderView != null) HeaderView.updateEditTextDirection(searchHeaderView!!.editView(), Screen.dp(68f), Screen.dp(49f))
        if (counterHeaderView != null) HeaderView.updateLayoutMargins(counterHeaderView, Screen.dp(68f), 0)
    ...
    protected fun navigationStack(): NavigationStack? {
        return if (navigationController != null) navigationController!!.getStack() else null
    }

    fun stackSize(): Int {
        return if (navigationController != null) navigationController!!.getStackSize() else 0
    }

    protected fun previousStackItem(): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().getPrevious() else null
    }

    fun stackItemAt(index: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().get(index) else null
    }

    fun removeStackItemAt(index: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().remove(index) else null
    }

    fun destroyStackItemAt(index: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().destroy(index) else null
    }
    ...
    fun removeStackItemById(id: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().removeById(id) else null
    }

    fun destroyStackItemById(id: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().destroyById(id) else null
    }
    ...
    fun destroyStackItemByIdExcludingLast(id: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().destroyByIdExcludingLast(id) else null
    }

    fun findLastStackItemById(id: Int): ViewController<*>? {
        return if (navigationController != null) navigationController!!.getStack().findLastById(id) else null
    }
    ...
    protected fun postNavigateBack() {
        // TODO proper way
        UI.post(Runnable { this.navigateBack() })
    }
  ...
  protected void setControllerAnimated (Class<? extends ViewController> rawController, Object args, boolean asForward, boolean saveFirst) {
    ...
    protected open fun onEnterSearchMode() {
        ...
        clearSearchInput(if (text == null) "" else text, true)
    }
    ...
    fun getArgumentsStrict (): T {
        return args ?: throw NullPointerException(toString() + " (" + javaClass.getSimpleName() + ") arguments are null")
    }
    ...
    protected fun getSelectedCount (): Int {
        return if (counterHeaderView != null) counterHeaderView!!.getCounter() else 0
    }
    ...
    protected fun getSearchHeaderView(headerView: HeaderView): SearchEditTextDelegate? {
        if (searchHeaderView == null) {
            ...
            params = newParams(ViewGroup.LayoutParams.MATCH_PARENT, Size.getHeaderPortraitSize())
    ...
    fun openUpdateAlert(text: CharSequence?) {
        openAlert(
            ...
            DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> Intents.openSelfGooglePlay() },
            0
        )
    }
    ...
    fun openAlert(@StringRes title: Int, message: CharSequence?): AlertDialog? {
        ...
        b.setPositiveButton(Lang.getOK(), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> dialog!!.dismiss() })
        return showAlert(b)
    }
    ...
    fun openAlert(@StringRes title: Int, message: CharSequence?, okListener: DialogInterface.OnClickListener?, needCancel: Boolean = true) {
        ...
        if (needCancel) {
            b.setNegativeButton(Lang.getString(R.string.Cancel), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> dialog!!.dismiss() })
        } else {
    ...
    fun openInputAlert(
        ...
        inputView.getEditText().setInputType(InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_WEB_EMAIL_ADDRESS)
        if (!isEmpty(value)) {
            inputView.setText(value)
            inputView.getEditText().setSelection(0, value!!.length)
        }
        inputView.getEditText().addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
            }
        ...
        val alert = AlertDialog.Builder(context, Theme.dialogTheme())
            .setTitle(title)
            .setView(ll)
            .setPositiveButton(Lang.getString(doneRes), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int ->
                val result = inputView.getText().toString()
            ...
            .setNegativeButton(Lang.getString(cancelRes), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int ->
                if (hideKeyboard) {
                    Keyboard.hide(inputView.getEditText())
                }
                dialog!!.dismiss()
        ...
        if (needReset) {
            alert.setNeutralButton(Lang.getString(R.string.ValueReset), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int ->
        ...
        if (button != null) button.setOnClickListener(View.OnClickListener { v: View? ->
            val result = inputView.getText().toString()
            if (callback.onAcceptInput(inputView, result)) {
                if (hideKeyboard) {
                    Keyboard.hide(inputView.getEditText())
    ...
    fun openMissingPermissionAlert(@StringRes message: Int) {
        ...
        b.setNegativeButton(
            ...
            DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> Intents.openPermissionSettings() })
        showAlert(b)
    }
    ...
    fun openMissingGoogleMapsAlert() {
        ...
        b.setPositiveButton(
            ...
            DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> Intents.openGooglePlay("com.google.android.apps.maps") })
        b.setNegativeButton(Lang.getString(R.string.Cancel), DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> dialog!!.dismiss() })
        showAlert(b)
    }
    ...
    fun openSecretLinkPreviewAlert(onAcceptWarning: RunnableBool?) {
        ...
        b.setPositiveButton(
            ...
            DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> after.runWithBool(true) }
        )
        b.setNegativeButton(
            Lang.getString(R.string.SecretLinkPreviewDisable),
            DialogInterface.OnClickListener { dialog: DialogInterface?, which: Int -> after.runWithBool(false) }
        )
        b.setCancelable(false)
    ...
    fun openLinkAlert(url: String, options: UrlOpenParameters?) {
        tdlib!!.ui().openUrl(this, url, if (options == null) UrlOpenParameters().requireOpenPrompt() else options.requireOpenPrompt())
    }
    ...
    fun showSettings(b: SettingsWrapBuilder): SettingsWrap? {
        ...
        if (b.needSeparators) {
            ...
            Collections.addAll<ListItem?>(items, *b.rawItems)
        }
        ...
        val recyclerView: RecyclerView = object : RecyclerView(context) {
            override fun onTouchEvent(e: MotionEvent): Boolean {
                if (e.getAction() == MotionEvent.ACTION_DOWN && settings.adapter != null && settings.adapter.getItemCount() > 0) {
                    val i = (getLayoutManager() as LinearLayoutManager).findFirstVisibleItemPosition()
                    if (i == 0) {
                        val view = getLayoutManager()!!.findViewByPosition(i)
            ...
            override fun onMeasure(widthSpec: Int, heightSpec: Int) {
                ...
                if (lastHeight != 0 && lastHeight != height) {
                    lastHeight = height
                    post(Runnable { this.invalidateItemDecorations() })
                } else {
                    lastHeight = height
                }
            }
            ...
            recyclerView.addItemDecoration(object : RecyclerView.ItemDecoration() {
                override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
                    ...
                    if (position == 0) {
                        ...
                        val controlsHeight = context.getRootView().getSystemInsetsWithoutIme().bottom + (if (b.disableFooter) 0 else Screen.dp(56f))
            ...
            if (firstPosition == 0) {
                ...
                if (view != null) {
                    return@PopupHeightProvider min(
                        ...
                        min(
                            settingsLayout.getMeasuredHeight() - view.getTop(),
        ...
        val onClickListener = View.OnClickListener { v: View? ->
            ...
            if (viewId == R.id.btn_cancel) {
                ...
                val type = settings.adapter.getCheckResultType()
        ...
        settings.adapter = object : SettingsAdapter(if (b.tdlibDelegate != null) b.tdlibDelegate else this, onClickListener, this) {
        ...
        if (!b.disableFooter) {
            ...
            for (i in 0..1) {
                ...
                if (i == 0) {
                    ...
                    button.setText(b.cancelStr.uppercase(Locale.getDefault()).also { text = it })
                    button.setLayoutParams(
                        newParams(
                            ViewGroup.LayoutParams.WRAP_CONTENT,
                            Screen.dp(55f),
                            (if (Lang.rtl()) Gravity.RIGHT else Gravity.LEFT) or Gravity.BOTTOM
                        )
                    )
                    settings.cancelButton = button
                } else {
                    button.setId(R.id.btn_save)
                    button.setText(b.saveStr.uppercase(Locale.getDefault()).also { text = it })
                    button.setLayoutParams(
                        newParams(
                            ViewGroup.LayoutParams.WRAP_CONTENT,
                            Screen.dp(55f),
                            (if (Lang.rtl()) Gravity.LEFT else Gravity.RIGHT) or Gravity.BOTTOM
                        )
                    )
        ...
        if (desiredHeight > Screen.currentActualHeight() && checkedIndex != -1) {
            val viewHeight = SettingHolder.measureHeightForType(items.get(checkedIndex)!!.getViewType())
    ...
    fun showCopyUrlOptions(url: String, options: UrlOpenParameters?, openCallback: FutureBool?) {
        ...
        showOptions(url, ids.get(), strings.get(), null, icons.get(), OptionDelegate { itemView: View?, id: Int ->
    ...
    fun showCallOptions(phoneNumber: String?, userId: Long) {
        ...
        showOptions(
            ...
            arrayOf<String?>(Lang.getString(R.string.PhoneCall), Lang.getString(R.string.VoipInCallBranding)),
    ...
    fun showConfirm(info: CharSequence?, okString: String?, okIcon: Int, okColor: Int, onConfirm: Runnable): PopupLayout? {
        return showOptions(
            ...
            arrayOf<String?>(if (okString != null) okString else Lang.getString(R.string.OK), Lang.getString(R.string.Cancel)),
    ...
    fun showUnsavedChangesPromptBeforeLeaving(info: CharSequence?, discardText: String, onConfirm: Runnable?) {
        showOptions(
            info,
            intArrayOf(R.id.btn_done, R.id.btn_cancel),
            arrayOf<String?>(discardText, Lang.getString(R.string.Cancel)),
    ...
    fun showWarning(info: CharSequence?, callback: RunnableBool): PopupLayout? {
        return showOptions(
            info,
            intArrayOf(R.id.btn_done, R.id.btn_cancel),
            arrayOf<String?>(Lang.getString(R.string.TdlibLogsWarningConfirm), Lang.getString(R.string.Cancel)),
            intArrayOf(
    ...
    class Options(@JvmField val info: CharSequence?, val title: CharSequence?, @JvmField val subtitle: OptionItem?, @JvmField val items: Array<OptionItem>, val maxLineCount: Int) {
        ...
        class Builder {
            ...
            fun items(ids: IntArray, titles: Array<String?>, colors: IntArray?, icons: IntArray?): Builder {
                for (i in ids.indices) {
                    val item = OptionItem.Builder()
                        .id(ids[i])
                        .name(titles[i])
                        .color(if (colors != null) colors[i] else OptionColor.NORMAL)
                        .icon(if (icons != null) icons[i] else 0)
                        .build()
                    items.add(item)
                }
                return this
            ...
            fun build(): Options {
                return ViewController.Options(info, title, subtitle, items.filterNotNull().toTypedArray(), maxLineCount)
            }
        }
    }
    ...
    fun showOptions(options: Options, delegate: OptionDelegate?, forcedTheme: ThemeDelegate? = null): PopupLayout? {
        ...
        if (delegate != null) {
            ...
            onClickListener = View.OnClickListener { v: View? ->
                ...
                if (c is OptionDelegate && (c as OptionDelegate).onOptionItemPressed(v, v!!.getId())) {
                    popupLayout.hideWindow(true)
                }
            }
        }
        var totalHeight = shadowView.getLayoutParams().height + optionsWrap.getTextHeight() + extraBottomInsetWithoutIme
        var index = 0
        for (item in options.items) {
            ...
            if (forcedTheme != null) Theme.forceTheme(text, forcedTheme)
    ...
    fun showText(title: CharSequence?, text: CharSequence?, entities: Array<TextEntity?>?, forcedTheme: ThemeDelegate?): PopupLayout {
        return showPopup(title, true, Text.LINE_COUNT_UNLIMITED, PopUpBuilder { popupLayout: PopupLayout?, optionsLayout: OptionsLayout? ->
    ...
    fun showCalendarDatePicker(
        ...
        val columns = Arrays.asList<ColumnDataPicker.Column?>(
            dayColumn,
            monthColumn,
            yearColumn
        )

        val getDay: FutureInt = object : FutureInt { override fun getIntValue (): Int = dayColumn.index + 1 }
        val getMonth: FutureInt = object : FutureInt { override fun getIntValue (): Int = monthColumn.index }
        val getYear: FutureInt = object : FutureInt { override fun getIntValue (): Int = if (yearOptional && yearColumn.index == yearColumn.rows.size - 1) 0 else minYear + yearColumn.index }
        ...
        dayColumn.setMinMaxProvider(MinMaxProvider { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
            var index = index
            val newMonth = getMonth.getIntValue()
            val newYear = getYear.getIntValue()
        ...
        dayColumn.setItemChangeListener(ItemChangeListener { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
        ...
        monthColumn.setMinMaxProvider(MinMaxProvider { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
            var index = index
            val newYear = getYear.getIntValue()
            if (newYear == maxYear) {
        ...
        dataPicker.showPopup(this, title, commitButtonText, null, CommitListener { picker: ColumnDataPicker?, commitButtonView: View? ->
            val newDay = getDay.getIntValue()
            val newMonth = getMonth.getIntValue()
            val newYear = getYear.getIntValue()
    ...
    fun showDateTimePicker(
        ...
        return showPopup(title, true, Text.LINE_COUNT_UNLIMITED, PopUpBuilder { popupLayout: PopupLayout?, optionsWrap: OptionsLayout? ->
            var contentHeight = 0
            ...
            setBackground(datePickerWrap, object : Drawable() {
                override fun draw(c: Canvas) {
                    ...
                    c.drawLine(
                        ...
                        viewWidth.toFloat(),
                        (cy - h2).toFloat(),
                        Paints.strokeSeparatorPaint(if (forcedTheme != null) forcedTheme.getColor(ColorId.separator) else Theme.separatorColor())
                    )
                    c.drawLine(
                        0f,
                        (cy + h2).toFloat(),
            ...
            for (day in startDay..<dayCount) {
                ...
                days.add(
                    SimpleStringItem(
                        0,
                        if (day == 0) Lang.getString(R.string.Today) else if (day == 1) Lang.getString(R.string.Tomorrow) else Lang.getDate(
                            startMillis,
            ...
            val calculateDate: FutureLong = object : FutureLong {
                override fun getLongValue (): Long {
                    ...
                    val year = dayItem.getArg1().toInt()
                    val dayOfYear = dayItem.getArg2().toInt()
                    val hour = hourItem.getArg1().toInt()
                    val minute = minuteItem.getArg1().toInt()
            ...
            dayPicker.setMinMaxProvider(MinMaxProvider { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
                ...
                for (minIndex in days.indices) {
                    if (days.get(minIndex)!!.getArg1() == year.toLong() && days.get(minIndex)!!.getArg2() == day.toLong()) {
                        return@MinMaxProvider max(minIndex, index)
                    }
                }
                index
            })
            dayPicker.setItemChangeListener(ItemChangeListener { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
            ...
            hourPicker.setMinMaxProvider(MinMaxProvider { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
            ...
            minutePicker.setMinMaxProvider(MinMaxProvider { v: InfiniteRecyclerView<SimpleStringItem?>?, index: Int ->
                val minDayIndex = dayPicker.getMinMaxProvider().getMinMax(v, 0)
            ...
            sendView.setOnClickListener(View.OnClickListener { v: View? ->
                val millis = calculateDate.getLongValue()
    ...
    fun tdlibId(): Int {
        return if (tdlib != null) tdlib.id() else TdlibAccount.NO_ID
    }
    ...
    fun openTdlibLogs(testerLevel: Int, crashInfo: Crash?) {
        showWarning(Lang.getMarkdownString(this, R.string.TdlibLogsWarning), RunnableBool { proceed: Boolean ->
            if (proceed) {
    ...
    fun openExperimentalSettings(testerLevel: Int) {
        showWarning(Lang.getMarkdownStringSecure(this, R.string.ExperimentalSettingsWarning), RunnableBool { proceed: Boolean ->
            if (proceed) {
                val c = SettingsBugController(context, tdlib)
    ...
    open fun onKeyboardStateChanged(visible: Boolean): Boolean {
        var currentPopup: View? = context.getCurrentPopupWindow()
    ...
    open fun onPrepareToShow() {
        if (inSearchMode()) {
            if ((flags and FLAG_PREVENT_LEAVING_SEARCH_MODE) == 0 && allowLeavingSearchMode()) {
                ...
                if (headerView != null && navigationController != null && navigationController!!.getStack()
                        .getCurrent() === this && headerView!!.getCurrentTransformMode() == HeaderView.TRANSFORM_MODE_SEARCH
                ) {
    ...
    fun addAttachStateListener(listener: AttachListener?) {
        if (attachListeners == null) {
            attachListeners = ReferenceList<AttachListener?>()
        }
  ...
  public void onLocaleChange () {
    ...
    protected fun updateSettingView(v: SettingView, item: ListItem, isUpdate: Boolean) {
        var value = instance().getNewSetting(item.getLongId())
        if (item.getBoolValue()) value = !value
    ...
    protected fun handleSettingClick(v: View, adapter: SettingsAdapter) {
        ...
        if (item.getBoolValue()) value = !value
        instance().setNewSetting(item.getLongId(), value)
    ...
    fun openInAppCamera(options: CameraOpenOptions) {
        if (options.allowSystem && instance().cameraType == Settings.CAMERA_TYPE_SYSTEM) {
            showOptions(
                ...
                arrayOf<String?>(Lang.getString(R.string.TakePhoto), Lang.getString(R.string.TakeVideo)),
                null,
