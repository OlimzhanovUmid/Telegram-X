...
abstract class BaseChartView<T : ChartData?, L : LineViewData?>(context: Context) : View(context), ChartPickerDelegate.Listener, ThemeInvalidateListener {
    ...
    private val pickerHeightUpdateListener: AnimatorUpdateListener = object : AnimatorUpdateListener {
    ...
    private val pickerMinHeightUpdateListener: AnimatorUpdateListener = object : AnimatorUpdateListener {
        override fun onAnimationUpdate(animation: ValueAnimator) {
    ...
    private val selectionAnimatorListener: AnimatorUpdateListener = object : AnimatorUpdateListener {
        override fun onAnimationUpdate(animation: ValueAnimator) {
    ...
    protected open fun init() {
        ...
        whiteLinePaint.setStrokeWidth(Screen.dpf(3f))
    ...
    protected open fun createLegendView(): LegendSignatureView {
        return LegendSignatureView(getContext())
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (getMeasuredWidth() != lastW || getMeasuredHeight() != lastH) {
            ...
            bottomChartBitmap = Bitmap.createBitmap(getMeasuredWidth() - (horizontalPadding shl 1), pikerHeight, Bitmap.Config.ARGB_4444)
            bottomChartCanvas = Canvas(bottomChartBitmap!!)

            sharedUiComponents!!.getPickerMaskBitmap(pikerHeight, getMeasuredWidth() - horizontalPadding * 2)
            measureSizes()
    ...
    private fun measureSizes() {
        ...
        pickerWidth = getMeasuredWidth() - (horizontalPadding * 2)
        chartStart = horizontalPadding
        chartEnd = getMeasuredWidth() - horizontalPadding
        ...
        chartArea.set(chartStart - horizontalPadding, 0, chartEnd + horizontalPadding, getMeasuredHeight() - chartBottom)
    ...
    private fun measureHeightThreshold() {
        val chartHeight = getMeasuredHeight() - chartBottom
    ...
    open fun drawBottomSignature(canvas: Canvas) {
        ...
        tmpI = 0
        while (tmpI < tmpN) {
            val resultAlpha = bottomSignatureDate.get(tmpI).alpha
            var step = bottomSignatureDate.get(tmpI).step
            if (step == 0) step = 1
            ...
            while (i < end) {
                ...
                if (xPointOffset > 0 &&
                    ...
                    canvas.drawText(
                        ...
                        xPoint,
                        (getMeasuredHeight() - chartBottom + BOTTOM_SIGNATURE_TEXT_HEIGHT + Screen.dp(3f)).toFloat(),
    ...
    protected open fun drawBottomLine(canvas: Canvas) {
        ...
        val textOffset = (signatureTextHeight - signaturePaint.getTextSize()).toInt()
        val y = (getMeasuredHeight() - chartBottom - 1)
        canvas.drawLine(
            chartStart.toFloat(),
            y.toFloat(),
            chartEnd.toFloat(),
    ...
    protected open fun drawSelection(canvas: Canvas) {
        ...
        if (drawPointOnSelection) {
            ...
            tmpI = 0
            while (tmpI < tmpN) {
                val line: LineViewData = lines!!.get(tmpI)!!
                ...
                val yPoint: Float = getMeasuredHeight() - chartBottom - (yPercentage) * (getMeasuredHeight() - chartBottom - signatureTextHeight)
    ...
    protected open fun drawHorizontalLines(canvas: Canvas, a: ChartHorizontalLinesData) {
        ...
        val chartHeight: Int = getMeasuredHeight() - chartBottom - signatureTextHeight
        for (i in (if (useMinHeight) 0 else 1) until n) {
            val y = ((getMeasuredHeight() - chartBottom) - chartHeight * ((a.values[i] - currentMinHeight) / (currentMaxHeight - currentMinHeight))).toInt()
    ...
    fun drawPicker(canvas: Canvas) {
        ...
        val bottom: Int = getMeasuredHeight() - pickerPadding
        val top: Int = getMeasuredHeight() - pikerHeight - pickerPadding
        ...
        if (chartData != null) {
            ...
            if (instantDraw) {
                canvas.save()
                canvas.clipRect(
                    horizontalPadding, getMeasuredHeight() - pickerPadding - pikerHeight,
                    getMeasuredWidth() - horizontalPadding, getMeasuredHeight() - pickerPadding
                )
                canvas.translate(horizontalPadding.toFloat(), (getMeasuredHeight() - pickerPadding - pikerHeight).toFloat())
            ...
            if (!instantDraw) {
                if (transitionMode == TRANSITION_MODE_PARENT) {
                    ...
                    canvas.save()
                    canvas.clipRect(horizontalPadding, top, getMeasuredWidth() - horizontalPadding, bottom)
                    canvas.scale(1 + 2 * transitionParams!!.progress, 1f, pX, pY)
                    canvas.drawBitmap(
                        bottomChartBitmap!!,
                        horizontalPadding.toFloat(),
                        (getMeasuredHeight() - pickerPadding - pikerHeight).toFloat(),
                        emptyPaint
                    )
                    canvas.restore()
                } else if (transitionMode == TRANSITION_MODE_CHILD) {
            ...
            canvas.drawRect(
                (end - DP_12).toFloat(),
                top.toFloat(),
                (getMeasuredWidth() - horizontalPadding).toFloat(),
                bottom.toFloat(), unactiveBottomChartPaint
            )
        } else {
            canvas.drawRect(
        ...
        canvas.drawBitmap(
            sharedUiComponents!!.getPickerMaskBitmap(pikerHeight, getMeasuredWidth() - horizontalPadding * 2),
            horizontalPadding.toFloat(), (getMeasuredHeight() - pickerPadding - pikerHeight).toFloat(), emptyPaint
        )
    ...
    protected fun setMaxMinValue(newMaxHeight: Int, newMinHeight: Int, animated: Boolean, force: Boolean, useAnimator: Boolean) {
        ...
        for (i in 0..<n) {
            val a = horizontalLines.get(i)
            if (a !== newData) a.fixedAlpha = a.alpha
        }

        alphaAnimator = createAnimator(0f, 255f, AnimatorUpdateListener { animation: ValueAnimator? ->
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        var x = event.getX(event.getActionIndex()).toInt()
        var y = event.getY(event.getActionIndex()).toInt()

        when (event.getActionMasked()) {
            MotionEvent.ACTION_DOWN -> {
                capturedTime = System.currentTimeMillis()
                getParent().requestDisallowInterceptTouchEvent(true)
                val captured = pickerDelegate.capture(x, y, event.getActionIndex())
                if (captured) {
                    return true
                }
            ...
            MotionEvent.ACTION_POINTER_DOWN -> return pickerDelegate.capture(x, y, event.getActionIndex())
            MotionEvent.ACTION_MOVE -> {
                val dx = x - lastX
                val dy = y - lastY

                if (pickerDelegate.captured()) {
                    val rez = pickerDelegate.move(x, y, event.getActionIndex())
                ...
                if (chartCaptured) {
                    ...
                    if (canCaptureChartSelection && System.currentTimeMillis() - capturedTime > 200) {
                        ...
                        disable = abs(dx) > abs(dy) || abs(dy) < touchSlop
                    }
                    lastX = x
                    lastY = y
    ...
    fun moveLegend(offset: Float = chartFullWidth * (pickerDelegate.pickerStart) - horizontalPadding) {
        ...
        if (lXPoint > (chartStart + chartWidth) shr 1) {
            lXPoint -= (legendSignatureView!!.getWidth() + DP_5).toFloat()
        } else {
        ...
        if (lXPoint < 0) {
            ...
            lXPoint = (getMeasuredWidth() - legendSignatureView!!.getMeasuredWidth()).toFloat()
        }
    ...
    open fun findMaxValue(startXIndex: Int, endXIndex: Int): Int {
        ...
        for (j in 0..<linesSize) {
            ...
            val lineMax = lines!!.get(j)!!.line.segmentTree.rMaxQ(startXIndex, endXIndex)
    ...
    open fun findMinValue(startXIndex: Int, endXIndex: Int): Int {
        ...
        for (j in 0..<linesSize) {
            if (!lines!!.get(j)!!.enabled) continue
            val lineMin = lines!!.get(j)!!.line.segmentTree.rMinQ(startXIndex, endXIndex)
    ...
    private fun updateDates(step: Int) {
        ...
        if (currentBottomSignatures == null || step >= currentBottomSignatures!!.stepMax || step <= currentBottomSignatures!!.stepMin) {
            ...
            tmpN = bottomSignatureDate.size
            for (i in 0..<tmpN) {
                val a = bottomSignatureDate.get(i)
            ...
            alphaBottomAnimator = createAnimator(0f, 1f, AnimatorUpdateListener { animation: ValueAnimator? ->
    ...
    open fun onCheckChanged() {
        ...
        tmpI = 0
        while (tmpI < tmpN) {
            val lineViewData: LineViewData = lines!!.get(tmpI)!!
            ...
            if (lineViewData.enabled && lineViewData.alpha != 1f) {
                ...
                lineViewData.animatorIn = createAnimator(lineViewData.alpha, 1f, AnimatorUpdateListener { animation: ValueAnimator? ->
            ...
            if (!lineViewData.enabled && lineViewData.alpha != 0f) {
                ...
                lineViewData.animatorOut = createAnimator(lineViewData.alpha, 0f, AnimatorUpdateListener { animation: ValueAnimator? ->
                    lineViewData.alpha = (animation!!.getAnimatedValue() as Float)
                    invalidatePickerChart = true
                    invalidate()
                })
